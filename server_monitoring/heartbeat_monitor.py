import asyncio
import json
import pandas as pd
from asyncio.subprocess import PIPE
from sqlalchemy import create_engine

# ---- Config ----

ps_script_path = r"C:\Scripts\monitor_agent.ps1"
servers = [...]  # Your full list of 3000+ servers

db_uri = "mysql+pymysql://your_user:your_pass@your_db_host/your_database"
engine = create_engine(db_uri)

BATCH_SIZE = 200  # Number of servers per batch

# ---- Async Function to Monitor a Server ----

async def monitor_server(server):
    process = await asyncio.create_subprocess_exec(
        "powershell", "-File", ps_script_path, "-ServerName", server,
        stdout=PIPE, stderr=PIPE
    )
    stdout, stderr = await process.communicate()

    if stderr:
        return {
            "server_name": server,
            "status": "Error",
            "action": stderr.decode().strip()
        }
    else:
        try:
            data = json.loads(stdout.decode().strip())
            return {
                "server_name": data.get("Server", server),
                "status": data.get("Status", "Unknown"),
                "action": data.get("Action", "Unknown")
            }
        except json.JSONDecodeError:
            return {
                "server_name": server,
                "status": "Error",
                "action": "Invalid JSON from PowerShell"
            }

# ---- Process One Batch ----

async def process_batch(server_batch):
    tasks = [monitor_server(server) for server in server_batch]
    results = await asyncio.gather(*tasks)
    return results

# ---- Run All Batches Sequentially ----

def chunk_list(data, chunk_size):
    for i in range(0, len(data), chunk_size):
        yield data[i:i + chunk_size]

def run_batches(all_servers):
    all_results = []

    for i, batch in enumerate(chunk_list(all_servers, BATCH_SIZE), 1):
        print(f"Processing batch {i} ({len(batch)} servers)...")
        batch_results = asyncio.run(process_batch(batch))
        all_results.extend(batch_results)

    return all_results

# ---- Run Script and Insert to DB ----

if __name__ == "__main__":
    results = run_batches(servers)

    # Convert to DataFrame
    df = pd.DataFrame(results)
    df['log_time'] = pd.Timestamp.now()

    # Insert into DB
    try:
        df.to_sql("agent_status_log", con=engine, if_exists='append', index=False)
        print(f"Inserted {len(df)} rows into MySQL.")
    except Exception as e:
        print(f"DB insert failed: {e}")
